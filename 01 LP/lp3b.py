...
LP Formulation using set notation & Rules:
...
# define objective function rule
def objective_rule(model):
    return sum(model.P[i] * model.x[i] for i in model.I)

# define the objective function using the rule
model.obj = Objective(rule=objective_rule, sense=maximize)

# define the constraints using rules
def constraint1_rule(model,i):
    return sum(model.L[i] * model.x[i] for i in model.I) <= 48
def constraint2_rule(model,i):
    return sum(model.F[i] * model.x[i] for i in model.I) <= 20
def constraint3_rule(model,i):
    return sum(model.C[i] * model.x[i] for i in model.I) <= 8
def constraint4_rule(model,i):
    return model.x["Table"] <= 5

# define the constraints
model.c1 = Constraint(model.I, rule = constraint1_rule)
model.c2 = Constraint(model.I, rule = constraint2_rule)
model.c3 = Constraint(model.I, rule = constraint3_rule)
